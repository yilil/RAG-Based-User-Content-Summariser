...
├─ src/
│  ├─ components/
│  │  ├─ Sidebar.tsx              # Platform & model selector
│  │  ├─ QuestionTemplates.tsx    # Auto-suggest popup
│  │  └─ … (shared UI atoms)
│  ├─ pages/
│  │  └─ SummaryPage.tsx          # Chat-like search UI
│  ├─ hooks/                      # Custom React hooks
│  ├─ utils/                      # Tiny helpers (fetch, scroll, etc.)
│  └─ main.tsx                    # React entry-point
├─ vite.config.ts                 # Vite dev & build setup
└─ index.html                     # Single-page shell

Frontend/ – Vite + React (TypeScript) single-page app
src/pages/SummaryPage.tsx – chat-style search screen (conversation, templates, auto-scroll)
src/components/Sidebar.tsx – model/platform selector, chat list, session-aware
src/components/QuestionTemplates.tsx – dynamic suggestions shown above the input, hidden on blur
src/lib/session.ts – tiny helper that fetches/ stores the session id cookie and re-uses it in every fetch.
vite.config.ts – proxy /api → http://127.0.0.1:8000/ to avoid CORS during dev
...
# static build (for prod container / Netlify)
npm run build          # outputs to Frontend/dist

3. Networking contract
All API calls hit /api/* – Vite rewrites to Django (8000)
Authentication is cookie-based only.
fetch("/api/search/", {
  		method: "POST",
  		credentials: "include",      // <-- crucial: sends/receives sessionid
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({...})
})
The first time the SPA mounts it calls /api/sessionKey/ (credentials = include); the Set-Cookie from Django is then reused automatically.


4. State flow
Sidebar dispatches newChat(platform) → React Context stores an object {chatId, platform, messages: []} in localStorage.
SummaryPage renders that chat.
On submit → onUpdateMessages("User: ..."), fetch, then onUpdateMessages("Bot: ...").
After each fetch we also call /api/getMemory/?session_id=${sessionId} to hydrate local history.


5. UX details implemented
Auto-scroll to bottom (latest) after every new message, but auto-scroll to top when the user switches chats.
QuestionTemplates appear above the bar, absolute-positioned; disappears on blur (150 ms delay to allow click).
Width: parent div is max-w-screen-lg mx-auto so the bar & messages snap to full width immediately (no intermediate fixed 300 px phase).
Accessible: input labelled, buttons have aria-label, prefers-color-scheme aware (Tailwind dark mode).

### 6. Style & Theme — quick UI customisation guide

#### 6.1 Tailwind entry-point
* Global stylesheet: `Frontend/src/index.css`
* Tailwind config: `tailwind.config.ts`
  1. Defines two brand colours (`primary`, `accent`);
  2. Enables `darkMode:"class"` — add `<html class="dark">` to switch to dark theme.

#### 6.2 Changing width / layout
Whenever you see  
```tsx
<div className="max-w-screen-lg mx-auto px-4">
max-w-screen-lg caps the container width
Change to max-w-screen-xl or remove it for full-width
mx-auto keeps it centred
px-4 is 1 rem horizontal padding (1 = 0.25 rem)


6.3 Moving / styling the search bar
The bar lives in SummaryPage.tsx:
<form className="sticky bottom-0 bg-white dark:bg-zinc-900 ...">
Replace sticky bottom-0 with fixed bottom-4 left-1/2 -translate-x-1/2 to float it above the page
Add shadow-lg rounded-full for a ChatGPT-like pill


6.4 Tweaking chat bubbles
See src/components/ChatBubble.tsx:
<div className={`p-3 rounded-lg ${isUser
  ? "bg-primary-500 text-white ml-auto"
  : "bg-zinc-200"}`}>
Change bg-primary-500 → bg-rose-500 for a pink user bubble
Add prose class to auto-format Markdown


6.5 Adding a new colour theme
...
Then in components just use bg-brand, hover:bg-brand-light, etc.
Heads-up: After editing tailwind.config.ts you must restart npm run dev to see the changes.
...
(1), 前后端联调：(这里标注为(1)方便区分，合并在项目readme文件里的时候删掉)
