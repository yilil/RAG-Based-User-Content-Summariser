...
import time
import random
from search.models import LittleRedBookContent

def get_xhs_content(url, headers):
    """
...
def save_to_db(data):
    ...
    if data:
        try:
            # 存储到数据库
            LittleRedBookContent.objects.create(
                source='xiaohongshu',
...
def fetch_and_save_xhs_content(url, headers):
    """
    集成整个爬取流程 -> 获取并保存小红书的内容
    """
    print(f"Fetching content from {url}...")
    data = get_xhs_content(url, headers)
    save_to_db(data)
